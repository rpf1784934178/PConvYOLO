...
def main():
    ...
    models = {
        "Baseline (YOLO11n)": baseline_pt,
        "PConv-YOLO (900e)": ours_pt
    }
        ...
        for model_name, pt_path in models.items():
            ...
            try:
                ...
                if mode == "val":
                    ...
                    metrics = model.val(data=yaml_file, split='test', imgsz=640,
                                        workers=0, verbose=False)
                    metric_value = metrics.box.map

                elif mode == "fit":
                    # 【关键修改】workers=0 解决内存溢出
                    print(f"   (正在进行 5 轮极速微调...)")
                    # batch=4 进一步降低显存压力
                    train_res = model.train(data=yaml_file, epochs=5, imgsz=640,
                                            workers=0, batch=4,
