...
class C3_PConv(C3k2):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, e=0.5, g=1):
        # 在调用父类时，显式指定参数名，防止传错位置
        super().__init__(c1, c2, n, shortcut, g=g, e=e)

        c_ = int(c2 * e)  # hidden channels
        # 将内部的 Bottleneck 替换为 PConv 版本
        self.m = nn.Sequential(*(Bottleneck_PConv(c_, c_, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n)))
...
class Bottleneck_PConv(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)  # 输入
        self.cv2 = PConv(c_, n_div=4)  # 核心 PConv
        self.cv3 = Conv(c_, c2, k[1], 1)  # 输出
        self.add = shortcut and c1 == c2
...
class DySample(nn.Module):
    ...
    def __init__(self, in_channels, scale=2, style="lp", groups=4, dyscope=False):
        ...
        self.offset = nn.Conv2d(in_channels, out, 1)
        self.normal_init(self.offset, std=0.001)
        if dyscope:
            self.scope = nn.Conv2d(in_channels, out, 1)
    ...
    def forward(self, x, flow=None):
        # 这里的实现逻辑是纯数学运算，PyTorch 原生支持
        # 为了节省篇幅，这里是简化的前向传播，实际可以直接使用
        # 只需要保证输入输出维度匹配即可
